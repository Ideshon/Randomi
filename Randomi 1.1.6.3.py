...
class TextRandomizerGUI(QWidget):
    ...
    # Рандомная выборка
    def expand_random_count(self, text, delim):
        pattern = rf'%(\d+)-(\d+)\((.*?)\)'
        while True:
            ...
            for (min_count, max_count, words) in match:
                words_list = [w.strip() for w in words.split(delim)]  # Удаление пробелов вокруг слов
                num_words = random.randint(int(min_count), min(int(max_count), len(words_list)))
    ...
    # функция нормализации примера (delim: "," ->  1,2,3 в {1|2|3})
    def normalize_template(self, template, delim):
        # Разделитель только для разделения слов, а не в качестве разделителя между вариантами
    ...
    def randomize_text(self):
        try:
            ...
            expanded_template = self.expand_random_count(expanded_template, self.delimeter.text()) #рандомная выборка
            norm_template = self.normalize_template(expanded_template, self.delimeter.text()) #нормализация разделителя
            text_rnd = TextRandomizer(norm_template) #рандомизация
            randomized_text = text_rnd.get_text() #вывод

            # Очистка от лишних знаков препинания и пробелов
            # Удаление запятых, не следующих сразу за словом
