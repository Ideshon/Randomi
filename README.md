...
Эти слова будут расставлены в случайном порядке.

$MULTIPLY(word, count) 
Эта функция будет повторять слово нужное количество раз. 
Эта функция работает после того, как сработают функции внутри нее, поэтому дублироваться будет весь результат и без рандомизации.
Например: $MULTIPLY($RANDWORDS(1, 2, a, b), 3) = b a b a b a. На данный момент можно использовать костыль: [+ +$MULTIPLY(a|,3)$MULTIPLY(b|,3)] = b  a a b  b   a (в результате будут лишние пробелы)
...
Размер шрифта(иногра не срабатывает без сброса форматирования), жирный шрифт, сброс форматирования.
