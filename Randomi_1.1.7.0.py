    ...
    def randomize_text(self):
        try:
            ...
            def replace_randwords(match):
                ...
                words = match.group(3)
                words = words.replace('|', ',')  # Заменяем '|' на ',' для разделения аргументов
                return '{$RANDWORDS(' + f'{min_count},{max_count},{words}' + ')}'
            template = re.sub(r'%(\d+)-(\d+)\((.*?)\)', replace_randwords, template)

            # Создание объекта TextRandomizer и добавление пользовательских функций
            text_rnd = TextRandomizer(template, parse=False)

            # Функция умножения слова
            def multiply(word, count):
                return ' '.join([word.strip()] * int(count))

            text_rnd.add_function('MULTIPLY', multiply)

            # Функция случайного выбора слов
            ...
            text_rnd.add_function('RANDWORDS', randwords)
