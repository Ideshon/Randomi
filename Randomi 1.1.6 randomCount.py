...
class TextRandomizerGUI(QWidget):
    ...
    # Рандомная выборка с удалением запятой после формулы при количестве слов 0
    def expand_random_count(self, text):
        ...
        while True:
            ...
            for (min_count, max_count, words) in match:
                ...
                num_words = random.randint(int(min_count), int(max_count))
                if num_words == 0:
                    selected_words = ''
                else:
    ...
    def normalize_template(self, template, delim):
        ...
        norm_template = sub(r"\s*;\s*", "|", template)
